...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Changing field 'RegistrationKey.user'
        profiles = []
        for key in orm.RegistrationKey.objects.all():
            try:
                profiles.append(key.user.get_profile())
            except:
                profiles.append(key.user)
        db.alter_column('bikeanjo_registrationkey', 'user_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['bikeanjo.Profile']))
        for i, key in enumerate(orm.RegistrationKey.objects.all()):
            key.user = profiles[i]
            key.save()
    ...
    def backwards(self, orm):
        ...
        # Changing field 'RegistrationKey.user'
        db.alter_column('bikeanjo_registrationkey', 'user_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['auth.User']))
